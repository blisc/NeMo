...
class TransformerDecoderInferNM(TransformerDecoderNM):
    ...
    def input_ports(self):
        ...
        return {
            ...
            "ctc_predictions": NeuralType({0: AxisType(BatchTag), 1: AxisType(TimeTag)}),
        }
    ...
    def forward(self, input_ids_tgt, hidden_states_src, input_mask_src, input_mask_tgt, input_length, ctc_predictions):
        K = 10
        predictions = input_ids_tgt
        for i in range(K):
            output = predictions[0].cpu().numpy().tolist()
            # import ipdb; ipdb.set_trace()
            print(self.tokenizer.ids_to_text(output))
            # Need to predict input_length / K units on every loop
            hidden_states_tgt = self.embedding_layer(input_ids_tgt)
            hidden_states = self.decoder(hidden_states_tgt, input_mask_tgt, hidden_states_src, input_mask_src)
            # Projection
            logits = torch.matmul(hidden_states, self.embedding_layer.token_embedding.weight.T)
            # Grab top input_length / K
            # predictions = torch.argmax(logits, axis=-1)
            top_logits, predictions = torch.max(logits, axis=-1)
            # for loop for now
            # top_logits_indices = torch.zeros(top_logits.shape)
            max_len = max(input_length // K)
            indices = torch.zeros(top_logits.shape[0], max_len, dtype=torch.long)
            for j in range(top_logits.shape[0]):
                _, index = torch.topk(top_logits[j], input_length[j].item() // K * (i + 1), sorted=False)
                index = torch.nn.functional.pad(index, (0, max_len - len(index)))
                indices[j] = index
                # x = indices.view(0, len(index))
                # x = index
            # top_logits_indices = indices
            masked_chars = torch.zeros(predictions.shape, dtype=torch.long).cuda() + 4
            masked_chars.scatter_(1, indices.cuda(), predictions)
            predictions = masked_chars
            predictions[:, 0] = 2
            # fmt:off
            # import ipdb; ipdb.set_trace()
            # fmt: on
