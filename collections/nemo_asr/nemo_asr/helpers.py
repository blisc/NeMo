...
def monitor_asr_train_progress(tensors: list, labels: list, eval_metric='WER', tb_logger=None, logger=None):
    """
...
def process_evaluation_epoch(global_vars: dict, eval_metric='WER', tag=None, logger=None):
    ...
    if tag is None:
        if logger:
            ...
            logger.info(f"==========>>>>>>Evaluation {eval_metric}: " f"{wer*100 : 5.2f}%")
        else:
            print(f"==========>>>>>>Evaluation Loss: {eloss}")
            print(f"==========>>>>>>Evaluation {eval_metric}: " f"{wer*100 : 5.2f}%")
        return {"Evaluation_Loss": eloss, f"Evaluation_{eval_metric}": wer}
    else:
        if logger:
            logger.info(f"==========>>>>>>Evaluation Loss {tag}: {eloss}")
            logger.info(f"==========>>>>>>Evaluation {eval_metric} {tag}: " f"{wer*100 : 5.2f}%")
        else:
            print(f"==========>>>>>>Evaluation Loss {tag}: {eloss}")
            print(f"==========>>>>>>Evaluation {eval_metric} {tag}:" f" {wer*100 : 5.2f}%")
