...
def create_dags(jasper_params, args, nf):
    ...
    nf.logger.info("Num of params in encoder: {0}".format(jasper_encoder.num_weights))

    # Callbacks to print info to console and Tensorboard
    train_callback = nemo.core.SimpleLossLoggerCallback(
        tensors=[loss, predictions, transcript, transcript_len],
        print_func=partial(monitor_asr_train_progress, labels=vocab, logger=nf.logger),
    ...
    eval_callback = nemo.core.EvaluatorCallback(
        ...
        user_epochs_done_callback=partial(process_evaluation_epoch, logger=nf.logger),
...
def main():
    ...
    if args.test_after_training:
        ...
        if nf.world_size > 1:
            nf.logger.warning("Skipping beam search WER as it does not work " "if doing distributed training.")
        else:
