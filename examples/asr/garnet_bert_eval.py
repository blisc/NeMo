...
def parse_args():
    parser = argparse.ArgumentParser(
        parents=[nm_argparse.NemoArgParser()], description='GarNet with BERT', conflict_handler='resolve'
    )
    parser.set_defaults(
        checkpoint_dir=None,
        optimizer="novograd",
...
def create_dag_and_callbacks(args, garnet_params, neural_factory):
    ...
    # Defining nodes
    tokenizer = nemo_nlp.SentencePieceTokenizer(model_path=args.tokenizer_file)
    ...
    t_len_2 = int_to_seq2(x=decoder_in, length=t_len)
    ...
    log_probs = decoder(
        ...
        input_mask_tgt=t_len_2,
        input_length=t_len,
        ctc_predictions=ctc_predictions,
    )
    # log_probs = t_log_softmax(hidden_states=logits)
    eval_loss = loss(logits=log_probs, output_ids=decoder_out, output_mask=out_mask)
...
def main():
    ...
    tensors, tokenizer = create_dag_and_callbacks(args, garnet_params, neural_factory)

    # Optimize
    start = time.time()
    evaluated_tensors = neural_factory.infer(tensors=list(tensors), checkpoint_dir=args.load_dir,)
