...
class TestASRPytorch(NeMoUnitTest):
    ...
    def test_transcript_normalizers(self):
        ...
        for i, s in enumerate(normalized_strings):
            self.assertTrue(
                manifest[i]["tokens"]
                == manifest.tokenize_transcript(s, manifest.labels_map, manifest.unk_index, manifest.blank_index)
            )
    ...
    def test_preprocessor_errors(self):
        def create_broken_preprocessor_1():
            dl = nemo_asr.AudioToMelSpectrogramPreprocessor(window_size=2, n_window_size=2)

        def create_broken_preprocessor_2():
            dl = nemo_asr.AudioToMelSpectrogramPreprocessor(window_stride=2, n_window_stride=2)

        def create_broken_preprocessor_3():
            dl = nemo_asr.AudioToMelSpectrogramPreprocessor(n_window_stride=2)

        def create_good_preprocessor_1():
            dl = nemo_asr.AudioToMelSpectrogramPreprocessor(window_size=0.02, window_stride=0.01)
